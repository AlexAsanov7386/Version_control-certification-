...
```sh
<<<<<<< HEAD
git branch -d «название ветки, которую надо удалить»(теперь пробуем создать конфликт веток)
=======
git branch -d «название ветки, которую надо удалить» и тут же добавляем ещё одну фразу; проверяем
